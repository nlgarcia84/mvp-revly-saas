...
const SettingsPage = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  useEffect(() => {
    if (id) {
      getBusinessProfileStatus(id).then(setBpStatus).catch(() => setBpStatus(null));
    }
  }, [id]);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className={`${nCard} p-6 flex flex-col gap-5`}>
        <div>
          ...
          <p className="text-xs text-neutral-400 leading-relaxed">
            Conecta tu cuenta de Google Business Profile para ver
            todas las reseñas de tu negocio (sin límite de 5).
            Necesitas tener el perfil verificado en Google.
          </p>
        </div>
        ...
        {bpStatus === null ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-xs text-neutral-400">
              Las reseñas se cargarán desde Business Profile API
              (todas disponibles). Si hay algún problema, se usará
              automáticamente Google Places API (5 reseñas) como
              respaldo.
            </p>
          </div>
        ) : (
          <Button as="a" href={`/api/google-business/connect?businessId=${id}`} variant="primary">
